# 2D memory-throughput reference + linear diffusion -- Metal variant.
...
# const backend = CPU()
using Metal; const backend = MetalBackend(); const FT = Float32 # Apple GPUs have no Float64

# no-flux (∂n = 0) ghost-node mirror -- identical to memcopy2D_KA.jl
...
# ---- Int32 shift/mask indexing (Metal workaround) ----
@kernel inbounds = true unsafe_indices = true function k_memcopy_fast!(A, B, mask::Int32, sh::Int32)
...
@kernel inbounds = true unsafe_indices = true function k_saxpy_fast!(A, B, C, s, mask::Int32, sh::Int32)
    I  = Int32(@index(Global, Linear)) - Int32(1)
    ix = (I & mask) + Int32(1)
    iy = (I >> sh)  + Int32(1)
    A[ix, iy] = B[ix, iy] + s * C[ix, iy]
...
@kernel inbounds = true unsafe_indices = true function k_diffusion_fast!(C2, C, dtD, mask::Int32, sh::Int32, nx::Int32, ny::Int32)
    I  = Int32(@index(Global, Linear)) - Int32(1)
    ix = (I & mask) + Int32(1)
    iy = (I >> sh)  + Int32(1)
    C2[ix, iy] = C[ix, iy] + dtD * lap(C, ix, iy, nx, ny)
...
# build the three kernels for a given n, with static ndrange, in either indexing style
function kernels(n; fast)
    if fast
        ispow2(n)          || error("fast=true needs n a power of two (got n=$n)")
        (n*n) % 256 == 0   || error("fast=true needs n*n divisible by 256")
        msk, sh  = Int32(n - 1), Int32(trailing_zeros(n))
        n32      = Int32(n)
        km = k_memcopy_fast!(backend, 256, (n*n,))
        ks = k_saxpy_fast!(backend, 256, (n*n,))
        kd = k_diffusion_fast!(backend, 256, (n*n,))
        return ((A, B)         -> km(A, B, msk, sh),
                (A, B, C, s)   -> ks(A, B, C, s, msk, sh),
                (C2, C, dtD)   -> kd(C2, C, dtD, msk, sh, n32, n32))
    else
...
function memcopy_bench(; ns=2 .^ (9:12), fast=true)   # 512 … 4096
    @printf("%s / %s   indexing: %s\n\n", backend_info(backend), FT,
...
function diffusion2D(; n=2048, nt=200, do_visu=false, fast=true)
    nx = ny = n                      # square domain
    D  = FT(1.0)
    dt = FT(1.0) / D / FT(4.1)          # dx² = 1
    @printf("%s / %s   nx=%d ny=%d   dt=%.5g   indexing: %s\n", backend_info(backend), FT,
            nx, ny, dt, fast ? "Int32 shift/mask" : "2D NTuple")
...
memcopy_bench()
# memcopy_bench(; fast=false)   # 2D NTuple indexing, for the ~5x comparison
# diffusion2D()
