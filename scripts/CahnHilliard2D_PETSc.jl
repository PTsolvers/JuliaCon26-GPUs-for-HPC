...
const w      = 0.03                 # interface width -> ~11 features across box
const γ      = PetscScalar(w^2 / 8) # gradient-energy coefficient
...
dx   = PetscScalar(lx / nx)
dy   = PetscScalar(ly / ny)

if rank == 0
    println("Cahn-Hilliard 2D, implicit (backward Euler) on a PETSc DMDA")
    @printf("  grid      : %d x %d  (%d MPI ranks)\n", nx, ny, nranks)
    @printf("  dt        : %.3e   (%d steps, t_end = %.3e)\n", dt_0, nt, nt * dt_0)
    # Explicit-scheme limit, for reference: this is what the fully explicit
    # script in CahnHilliard2D.jl is forced to use.
    κmax = 4 / dx^2 + 4 / dy^2
    dt_expl = 2.0 / (D * κmax * (γ * κmax + 2.0)) / 2.0
    @printf("  dt_explicit ≈ %.3e   (implicit dt is %.0f× larger)\n", dt_expl, dt_0 / dt_expl)
end
...
PETSc.setfunction!(snes, r) do g_fx, snes_, g_x
    ...
    dx_ = PetscScalar(lx / mx_)
    dy_ = PetscScalar(ly / my_)
...
Print the global mass ∫C dV (should be constant to solver tolerance) and the
range of C, and update the GLMakie plot when running serially.
"""
function report(it, t)
    lsum, lmin, lmax, ln = PETSc.withlocalarray!(x; read = true, write = false) do x_arr
        Cv = @view x_arr[1:2:end]
        (sum(Cv), minimum(Cv), maximum(Cv), length(Cv))
    end
    gsum = MPI.Allreduce(lsum, +, comm)
    gmin = MPI.Allreduce(lmin, min, comm)
    gmax = MPI.Allreduce(lmax, max, comm)
    gn   = MPI.Allreduce(ln, +, comm)

    if rank == 0
        its = PETSc.LibPETSc.SNESGetIterationNumber(petsclib, snes)
        @printf("step %5d  t = %.4e  ⟨C⟩ = %.10f  C ∈ [%+.4f, %+.4f]  Newton its = %d\n",
                it, t, gsum / gn, gmin, gmax, its)
    end
...
if CreatePlots
    save("CahnHilliard2D_PETSc.png", fig)
    if !isinteractive()
